"""Disaggregate true on-policy for Qwen3-30B-A3B (MoE).

4 actor GPUs + 4 rollout GPUs = 8 GPUs total (no --colocate).
Uses train_async.py for asynchronous generation + training.
"""
import math
import os
...
# Disaggregate GPU split: actor + rollout = NUM_GPUS
ACTOR_NUM_GPUS = int(os.environ.get("SLIME_ACTOR_NUM_GPUS", "4"))
ROLLOUT_NUM_GPUS = int(os.environ.get("SLIME_ROLLOUT_NUM_GPUS", "4"))
assert ACTOR_NUM_GPUS + ROLLOUT_NUM_GPUS <= NUM_GPUS, (
    f"ACTOR_NUM_GPUS ({ACTOR_NUM_GPUS}) + ROLLOUT_NUM_GPUS ({ROLLOUT_NUM_GPUS}) > NUM_GPUS ({NUM_GPUS})"
)

USE_RAW = os.environ.get("SLIME_USE_RAW", "1") == "1"

# TP/EP for actor (Megatron)
TP_SIZE = int(os.environ.get("SLIME_TP_SIZE", str(ACTOR_NUM_GPUS)))
PP_SIZE = int(os.environ.get("SLIME_PP_SIZE", "1"))

# SGLang TP/EP override
SGLANG_TP_SIZE = int(os.environ.get("SLIME_SGLANG_TP_SIZE", str(ROLLOUT_NUM_GPUS)))
SGLANG_EP_SIZE = int(os.environ.get("SLIME_SGLANG_EP_SIZE", str(ROLLOUT_NUM_GPUS)))
...
# Staleness control for async rollout: drop samples generated with a weight
# version more than N updates behind the current training version.
# Set to "" or unset to disable.
ROLLOUT_MAX_STALENESS = os.environ.get("SLIME_ROLLOUT_MAX_STALENESS", "")
...
def execute():
    ...
    # Actor parallelism: TP * PP must fit in ACTOR_NUM_GPUS
    tensor_parallel_size = TP_SIZE
    pipeline_parallel_size = PP_SIZE
    assert ACTOR_NUM_GPUS % (tensor_parallel_size * pipeline_parallel_size) == 0, (
        f"ACTOR_NUM_GPUS ({ACTOR_NUM_GPUS}) must be divisible by TP * PP ({tensor_parallel_size} * {pipeline_parallel_size})"
    )
    data_parallel_size = ACTOR_NUM_GPUS // (tensor_parallel_size * pipeline_parallel_size)
    ...
    gpus_per_sglang_engine = sglang_tp_size
    ...
    wandb_args = (
        ...
        "--wandb-group qwen3-30B-A3B-megatron-disagg "
    ...
    if ROLLOUT_MAX_STALENESS:
        rollout_args += f"--rollout-max-staleness {ROLLOUT_MAX_STALENESS} "
    ...
    # CPU offload optimizer to free GPU memory for weight sync all-gather
    optimizer_args += (
    ...
    sglang_mem_fraction_static = float(
        _system_env("SLIME_SGLANG_MEM_FRACTION_STATIC", "0.5")
    )
    sglang_args = (
    ...
    # Disaggregate: no --colocate, separate actor and rollout GPUs
    ...
    misc_args += (
        ...
        # No --colocate for disaggregate mode
    )

    train_memory_margin_bytes = _system_int("SLIME_TRAIN_MEMORY_MARGIN_BYTES", 1024**3)
