...
class HfWeightIteratorDirect(HfWeightIteratorBase):
    ...
    def _convert_to_hf_named_tensors(self, megatron_full_params: Sequence[torch.Tensor], param_infos: list[ParamInfo]):
        ...
        for info, param in zip(param_infos, megatron_full_params, strict=False):
            ...
            # DEBUG: Print converted expert weight info for layer 47 (last layer)
            if debug_expert_sync and rank == 0 and "layers.47." in info.name and ".experts." in info.name:
                for hf_name, hf_tensor in converted:
                    ...
                    print(f"  Norm: {hf_tensor.float().norm().item():.10e}", flush=True)
